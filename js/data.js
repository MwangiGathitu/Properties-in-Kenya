async function loadHomepageData() {
    ...
    try {
        ...
        if (statProperties) {
            statProperties.textContent = allCount.toLocaleString();
            statProperties.classList.remove('loading');
        }
        
        const statAgents = document.getElementById('statAgents');
        if (statAgents) {
            statAgents.textContent = "150+";
            statAgents.classList.remove('loading');
        }
        
        const statSearches = document.getElementById('statSearches');
        if (statSearches) {
            statSearches.textContent = "12k+";
            statSearches.classList.remove('loading');
        }
        ...
        carousel.innerHTML = `
            <div style="padding:60px; text-align:center; color:#9CA3AF; width:100%;">
